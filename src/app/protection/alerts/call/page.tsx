...
import { Phone, ShieldAlert, Ban, CheckCircle, Flag, X, Loader2, Info } from "lucide-react";
...
export default function CallAlertPage() {
  ...
  const handleAction = (action: 'block' | 'trust' | 'report' | 'ignore') => {
    ...
    if (action === 'block') {
      ...
      toast({ title: "Número Confiável", description: "O número foi adicionado à tua lista branca." });
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-8 bg-blue-500 text-white space-y-4">
        ...
        <div className="flex items-center gap-2 text-blue-100">
           ...
           <span className="text-xs font-medium uppercase tracking-widest">Análise de IA local</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-900 text-white p-6 space-y-8 animate-in fade-in duration-500 overflow-y-auto pb-24">
      <div className="flex flex-col items-center text-center space-y-4 pt-8">
        ...
        <div className="space-y-1">
          <Badge variant="destructive" className="animate-pulse mb-2">RISCO DETETADO</Badge>
      ...
      <div className="bg-white/5 border border-white/10 p-4 rounded-3xl space-y-3">
        <div className="flex items-center gap-2 text-red-400">
          ...
          <h4 className="text-xs font-bold uppercase tracking-wider">Análise de Segurança</h4>
        </div>
        <div className="grid grid-cols-2 gap-2">
           <div className="bg-white/5 p-3 rounded-2xl border border-white/10">
              <span className="text-[10px] text-white/40 block mb-1">NÍVEL</span>
              <span className="font-bold text-red-400">{analysis?.riskLevel}</span>
           </div>
           <div className="bg-white/5 p-3 rounded-2xl border border-white/10">
              <span className="text-[10px] text-white/40 block mb-1">TIPO</span>
              <span className="font-bold text-white/90 truncate block">{analysis?.scamType}</span>
           </div>
        </div>
      ...
      <div className="space-y-4">
        <p className="text-center text-sm font-medium text-white/70 px-4">
          Este número parece ser fraudulento. Desejas bloqueá-lo agora?
        </p>
      ...
      <p className="text-center text-[10px] text-white/30 italic px-6 leading-relaxed">
        O ShieldCheck nunca bloqueia chamadas automaticamente. A decisão final é sempre tua.
      </p>
